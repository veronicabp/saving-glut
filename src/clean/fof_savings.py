...
def get_subcategory_shares(fof):
	...
	dfs = []
	funcs = [get_mufu_shares,get_mmf_shares,get_pension_shares,get_life_insurance_shares,get_ira_shares]
	for i, asset in enumerate(['Mutual Fund Shares', 'Money Market Fund Shares', 'Pension Entitlements', 'Life Insurance', 'IRA']):
...
def main():
	...
	# First, calculate for those where inflation is known and collapse
	...
	df['inflation_oth'] = (df['Amount'][True].sum(axis=1) - (df['resid_savings']) )/df['L_Amount'][True].sum(axis=1)

	for p in [1,9,90]:
	    df[p] = (df['Saving'][False][p] + df['Amount'][True][p] - df['inflation_oth']*df['L_Amount'][True][p])/df['NationalInc']
